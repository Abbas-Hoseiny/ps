...
type BbchSuggestion = {
  code: string;
  label: string;
  principalStage?: number;
  secondaryStage?: number;
};
const searchEppoSuggestions = async (
  _term: string,
  _limit: number
): Promise<EppoSuggestion[]> => [];
const searchBbchSuggestions = async (
  _term: string,
  _limit: number
): Promise<BbchSuggestion[]> => [];
